...
Expect `1.3.0+burke.9`. A bare `1.3.0` means PyPI upstream; anything `0.x` means
...
- [ ] `odoo-mcp --version` → **`odoo-mcp 1.3.0+burke.9`** (a bare `1.3.0` is the wrong build)
- [ ] `odoo-mcp --health` exits 0 and its JSON shows `"package_version": "1.3.0+burke.9"`
- [ ] In Claude, call `health_check` and confirm:
  - [ ] `package_version` is `1.3.0+burke.9`
