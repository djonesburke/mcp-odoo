...
- **Precedence**: when `ODOO_URL`/`ODOO_DB`/`ODOO_USERNAME` and a credential (`ODOO_API_KEY` or `ODOO_PASSWORD`) are all set, the environment wins and defines a single instance named `default` — unset them to use a multi-instance file. `ODOO_PASSWORD` is optional when `ODOO_API_KEY` is set, so an API-key deployment need not store the same secret under both names.
